...
type ResponsiveValue = GradualBlurProps[ResponsiveKey];
...
function getResponsiveValue(
  ...
  if (key === "height") {
    if (prefix === "mobile") return config.mobileHeight;
    if (prefix === "tablet") return config.tabletHeight;
    return config.desktopHeight;
  }

  if (prefix === "mobile") return config.mobileWidth;
  if (prefix === "tablet") return config.tabletWidth;
...
const useResponsiveDimension = (
  ...
  const [val, setVal] = useState<ResponsiveValue>(config[key]);
  useEffect(() => {
    if (!responsive) return;
    const calc = () => {
      const w = window.innerWidth;
      let v: ResponsiveValue = config[key];
      const mobileValue = getResponsiveValue(config, key, "mobile");
      const tabletValue = getResponsiveValue(config, key, "tablet");
      const desktopValue = getResponsiveValue(config, key, "desktop");
